...
class YoloTRT():
    ...
    def Inference(self, img):
        ...
        plate_bboxes = []  # List to store plate bounding boxes
        det_res = []  # List to store detection results
        for i in range(self.batch_size):
            ...
            for j in range(len(result_boxes)):
                box = result_boxes[j]
                plate_bboxes.append(box)  # Store bounding boxes
                det = dict()
        ...
        # Extract license plate numbers using plate_bboxes
        plate_numbers = []
        for plate_bbox in plate_bboxes:
            plate_img = self.extract_plate_region(image_raw, plate_bbox)
            plate_number = self.extract_plate_number(plate_img)
            plate_numbers.append(plate_number)

        return det_res, plate_numbers, t2 - t1
    ...
    def extract_plate_region(self, image, plate_bbox):
    ...
    def extract_plate_number(self, plate_img):
        ...
        plate_number = pytesseract.image_to_string(threshold_plate, lang='eng', config='--oem 3 --psm 7 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789')
        return plate_number
